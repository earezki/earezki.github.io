...
def _get_company_financial_data(ticker_symbol):
    company_data = {}
...
def get_financials(ticker, llm) -> str:
    prompt = PromptTemplate.from_template("""
    You are a financial data processor. Extract and structure **only the facts** from the provided company JSON.  
    **Do NOT interpret, predict, or recommend.**  
    **No opinions, no sentiment, no outlook.**
    ...
    **MANDATORY STRUCTURE (DO NOT DEVIATE):**
    ...
    - **Average Price Target**: ${{analyst_target}}
    - **Number of Analysts**: {{analyst_count}}

    ### Key Risks (Flags from Data)
    - [ ] Negative Net Income
    ...
    **RULES (STRICT):**
    - **Extract verbatim** from {{company_data}}. If field missing → `N/A`
    - **No sentences** — only **key: value** or **bullets**
    - **No analysis, no "strong", "weak", "bullish", "bearish"**
    - **No investment advice**
    - **No future predictions**
    - **No placeholders** — all fields must be filled or `N/A`
    - **No YAML, no code, no markdown beyond structure**
    """)

    company_data = _get_company_financial_data(ticker)
    chain = prompt | llm
    response = chain.invoke({
        "company_data": company_data,
    }).content

    print(f"[DEBUG] Financial report for {ticker}:\n{response}")
